...
pc-deployment-5ffc5bf56c-2ckdp   1/1     Running   0          69s   10.244.1.42   node02   app=nginx-pod
pc-deployment-5ffc5bf56c-8hfxp   1/1     Running   0          69s   10.244.1.41   node02   app=nginx-pod
pc-deployment-5ffc5bf56c-xkxtz   1/1     Running   0          69s   10.244.2.15   node03   app=nginx-pod

# 为了方便测试，修改下Nginx的index.html页面内容
[root@node01]> kubectl exec -it pc-deployment-5ffc5bf56c-2ckdp -n dev /bin/sh
> echo "node02: 10.244.1.42" > /usr/share/nginx/html/index.html
> exit
[root@node01]> kubectl exec -it pc-deployment-5ffc5bf56c-8hfxp -n dev /bin/sh
> echo "node02: 10.244.1.41" > /usr/share/nginx/html/index.html
> exit

[root@node01]> kubectl exec -it pc-deployment-5ffc5bf56c-xkxtz -n dev /bin/sh
> echo "node03: 10.244.2.15" > /usr/share/nginx/html/index.html
> exit

#修改完毕之后，访问测试
[root@node01]> curl 10.244.1.42
node02: 10.244.1.42
[root@node01]> curl 10.244.1.41
node02: 10.244.1.41
[root@node01]> curl 10.244.2.15
node03: 10.244.2.15
```
...
service-clusterip   ClusterIP   10.98.98.98   <none>        80/TCP    9s    app=nginx-pod
...
Port:              <unset>  80/TCP
TargetPort:        80/TCP
Endpoints:         10.244.1.41:80,10.244.1.42:80,10.244.2.15:80 # Service与Pod的映射规则
...
<img src=".images/image-20210602111215630.png" alt="image-20210602111215630" style="zoom:50%;" />
