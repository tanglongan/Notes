...
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    int hash = spread(key.hashCode());	// 计算出hash值
    int binCount = 0;					// 计算相应位置链表的长度
    for (Node<K,V>[] tab = table;;) {
        Node<K,V> f; int n, i, fh;
        
        if (tab == null || (n = tab.length) == 0){//1、数组为空，需要进行初始化
            tab = initTable();
        } else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {// 2、根据hash值计算元素在数组的位置，得到第一个节点f
            // 数组元素处为空，CAS操作放入元素，如果成功就结束了；否则就进入下一轮循环
            if (casTabAt(tab, i, null, new Node<K,V>(hash, key, value, null))){
                break;
            }
        } else if ((fh = f.hash) == MOVED){ //扩容，并迁移数据
                ...
                if (tabAt(tab, i) == f) {
                    ...
                    if (fh >= 0) {
                        ...
                        for (Node<K,V> e = f;; ++binCount) {
                            K ek;
                            //判断是否存在“key相等”的节点，如果存在，根据是否覆盖，进行更新。完成那个之后退出循环
            ...
            if (binCount != 0) {
                if (binCount >= TREEIFY_THRESHOLD){
                   //treeifyBin方法内不一定做红黑树转化。如果数组长度超过了64，就会转化为红黑树；否则就只是数组扩容。
                    treeifyBin(tab, i);
                }
                if (oldVal != null){
                    return oldVal;
...
主要是初始化一个合适大小的数组，然后会设置sizeCtl。初始化方法中的并发问题，通过对sizeCtl进行CAS控制进行的。
...
private final Node<K,V>[] initTable() {
    ...
    while ((tab = table) == null || tab.length == 0) {
        if ((sc = sizeCtl) < 0){
            Thread.yield(); // 初始化的工作被其他线程抢先了; 线程进入自旋。
        }else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {//CAS操作一下，将 sizeCtl设置为-1，代表抢到了锁
            try {
                if ((tab = table) == null || tab.length == 0) {
                    //DEFAULT_CAPACITY默认初始容量是16
                    int n = (sc > 0) ? sc : DEFAULT_CAPACITY;
                    //初始化数组，长度为16或初始化时提供的长度
...
#### 2、链表转化为红黑树treeifyBin

treeifyBin方法不一定就会进行红黑树转换，也可能是仅仅做数组扩容
